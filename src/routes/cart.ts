...
cartRouter.get("/", async (request: AuthRequest, response) =>
...
cartRouter.post("/items", async (request: AuthRequest, response) => {
  ...
  if (!(await Product.exists({ id: parsed.data.productId })))
    return response.status(404).json({ message: "Product not found" });

  let cart: any = await Cart.findOneAndUpdate(
  {
  ...
  {
    $inc: {
      "items.$.quantity": parsed.data.quantity,
    },
  },
  {
    new: true,
...
// If product was not found, create/add it
if (!cart) {
  ...
  if (!cart.items.some((item: any) => item.productId === parsed.data.productId)) {
    cart.items.push(parsed.data);
    await cart.save();
  }
  response.json(await getCart(request.ownerKey!));
});
cartRouter.patch(
  ...
  async (request: AuthRequest, response) => {
    const parsed = cartItemSchema.shape.quantity.safeParse(
      request.body.quantity,
    );
    if (!parsed.success)
      return response
        .status(400)
        .json({ message: "Positive integer quantity is required" });
    await Cart.findOneAndUpdate(
      { ownerKey: request.ownerKey },
      { $set: { "items.$[item].quantity": parsed.data } },
      { arrayFilters: [{ "item.productId": request.params.productId }] },
    );
    response.json(await getCart(request.ownerKey!));
  },
);
...
cartRouter.delete("/", async (request: AuthRequest, response) => {
  await Cart.findOneAndUpdate(
    { ownerKey: request.ownerKey },
    { $set: { items: [] } },
    { upsert: true },
